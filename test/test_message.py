...
def message_object():
    ...
    obj = (
        MessageBuilder.set_command(_command)
        .set_memory_area(_memory_area)
        .set_data(_data)
        .build()
    )
...
def test_byte_stream(message_object):
    _expected_byte_stream = (
        b"\x00\x00\x07\x57\x00\x01\x02\x03\x04\x05\x06\x07\x8E\x89\xD8\xC0\x03"
    )
